# Route & content outline

All content lives in `src/content/site.ts` (nav, hero, manifesto, projects, services, pantheon, proof, contact, socials).

| Route | Status | Built from | Next content needed |
| --- | --- | --- | --- |
| `/` | **Complete** | Hero · Manifesto · Work · Services · Pantheon · Interruption · Pause · Proof · Contact | — |
| `/work` | Foundation | PageIntro + all `ProjectCard`s + Contact | Filters by discipline once there are more than 4 projects |
| `/work/:slug` | Template | Case hero, full-bleed plate, body, next-project link | Challenge / idea / craft / outcome copy and 6–10 supporting images per project |
| `/services` | Foundation | PageIntro + Services index | One sub-section per discipline: approach, deliverables, related work |
| `/people` | Foundation | PageIntro + Pantheon | Per-god bios; also the mortal team, once there is one |
| `/culture` | Foundation | PageIntro + Interruption + Pause | Principles, rituals, open roles, events (Dionysus) |
| `/about` | Foundation | PageIntro + Manifesto + Proof | History, working model, collaborator network |
| `/contact` | Foundation | PageIntro + Contact | Real address, a brief form (name / company / problem / budget band), office hours |
| `*` | Done | 404 — "Not even Hermes found it." | — |

## Placeholders to replace before launch
- `hello@olympus.agency`: marked "Placeholder" in the contact section, footer and menu.
- Social links (`#`): Instagram, LinkedIn, Vimeo, Are.na.
- The four projects are fictional concepts, and the site says so in the Work intro and the footer.
- The collaborator cities in Proof are illustrative.
- Case-study body copy ("Full case study in production").
